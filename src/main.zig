...
fn init_logger(io: std.Io) ?Logger {
    ...
    const base = path_util.get_appdata_path(&appdata_buffer, "phantom") catch return null;

    var path_buffer: [path_util.path_length_max]u8 = undefined;

    const log_path = path_util.join_path(&path_buffer, base, "phantom.log") orelse return null;
