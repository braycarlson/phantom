...
build mode="debug":
    cls && just clean && zig build {{ if mode == "debug" { "" } else if mode == "release-safe" { "-Doptimize=ReleaseSafe" } else if mode == "release-fast" { "-Doptimize=ReleaseFast" } else if mode == "release-small" { "-Doptimize=ReleaseSmall" } else { "" } }} && zig-out\bin\phantom.exe
